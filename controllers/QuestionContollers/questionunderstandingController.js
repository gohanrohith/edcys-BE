const Question_Understanding = require('../../models/Questions/Question_Understanding');  // Updated import statement
...
const createQuestion = async (req, res) => {
    try {
        ...
        const newQuestion = new Question_Understanding({
            chapterId,
            question,
            options,
            correctAnswer,
...
const updateQuestion = async (req, res) => {
    try {
        ...
        const updatedQuestion = await Question_Understanding.findByIdAndUpdate(id, {
            question,
            options,
            correctAnswer,
            questionType,
...
const uploadMultipleQuestions = async (req, res) => {
    try {
        ...
        const newQuestions = await Question_Understanding.insertMany(questionsWithAdminId);
...
const getQuestionsByChapter = async (req, res) => {
    try {
        ...
        const allQuestions = await Question_Understanding.find({ chapterId }).populate('chapterId', 'name');
...
const getQuestionById = async (req, res) => {
    try {
        ...
        const question = await Question_Understanding.findById(id).populate('chapterId', 'name');
        if (!question) {
            return res.status(404).json({ message: 'Question not found' });
        }
...
const deleteQuestion = async (req, res) => {
    try {
        ...
        const deletedQuestion = await Question_Understanding.findByIdAndDelete(id);
        if (!deletedQuestion) {
            return res.status(404).json({ message: 'Question not found' });
        }
...
const getAllQuestionsByChapterId = async (req, res) => {
    try {
        ...
        // Fetch all questions for the chapter
        const questions = await Question_Understanding.find({ chapterId }).populate('chapterId', 'name');
