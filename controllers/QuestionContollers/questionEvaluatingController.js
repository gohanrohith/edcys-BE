const Question_Evaluating = require('../../models/Questions/Question_Evaluating');  // Updated import statement
...
const createQuestion = async (req, res) => {
    try {
        ...
        const newQuestion = new Question_Evaluating({
            chapterId,
            question,
            options,
            correctAnswer,
...
const updateQuestion = async (req, res) => {
    try {
        ...
        const updatedQuestion = await Question_Evaluating.findByIdAndUpdate(id, {
            question,
            options,
            correctAnswer,
            questionType,
...
const uploadMultipleQuestions = async (req, res) => {
    try {
        ...
        const newQuestions = await Question_Evaluating.insertMany(questionsWithAdminId);
...
const getQuestionsByChapter = async (req, res) => {
    try {
        ...
        const allQuestions = await Question_Evaluating.find({ chapterId }).populate('chapterId', 'name');
...
const getQuestionById = async (req, res) => {
    try {
        ...
        const question = await Question_Evaluating.findById(id).populate('chapterId', 'name');
        if (!question) {
            return res.status(404).json({ message: 'Question not found' });
        }
...
const deleteQuestion = async (req, res) => {
    try {
        ...
        const deletedQuestion = await Question_Evaluating.findByIdAndDelete(id);
        if (!deletedQuestion) {
            return res.status(404).json({ message: 'Question not found' });
        }
...
const getAllQuestionsByChapterId = async (req, res) => {
    try {
        ...
        // Fetch all questions for the chapter
        const questions = await Question_Evaluating.find({ chapterId }).populate('chapterId', 'name');
